...
impl KannalaBrandtModel {
    ...
    pub fn linear_estimation(
        ...
        for i in 0..num_points {
            ...
            if (self.intrinsics.fx * x_r).abs() < f64::EPSILON && x_r.abs() > f64::EPSILON {
                return Err(CameraModelError::NumericalError(
                    "fx * x_r is zero in linear estimation".to_string(),
                ));
            }
            if (self.intrinsics.fy * y_r).abs() < f64::EPSILON && y_r.abs() > f64::EPSILON {
                return Err(CameraModelError::NumericalError(
            ...
            if x_r.abs() > f64::EPSILON {
                b_vec[i * 2] = (u_img - self.intrinsics.cx) / (self.intrinsics.fx * x_r) - theta;
            } else {
                b_vec[i * 2] = if (u_img - self.intrinsics.cx).abs() < f64::EPSILON {
                    -theta
                } else {
                    0.0
                };
            ...
            if y_r.abs() > f64::EPSILON {
                b_vec[i * 2 + 1] =
                    (v_img - self.intrinsics.cy) / (self.intrinsics.fy * y_r) - theta;
            } else {
                b_vec[i * 2 + 1] = if (v_img - self.intrinsics.cy).abs() < f64::EPSILON {
                    -theta
                } else {
                    0.0
                };
        ...
        self.distortions = [x_coeffs[0], x_coeffs[1], x_coeffs[2], x_coeffs[3]];
