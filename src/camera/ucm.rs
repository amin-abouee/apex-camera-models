...
impl UcmModel {
    ...
    pub fn linear_estimation(
        ...
        for i in 0..num_points {
            ...
            let u_cx = u - self.intrinsics.cx;
            let v_cy = v - self.intrinsics.cy;

            a[(i * 2, 0)] = u_cx * (d - z);
            a[(i * 2 + 1, 0)] = v_cy * (d - z);

            b[i * 2] = (self.intrinsics.fx * x) - (u_cx * z);
            b[i * 2 + 1] = (self.intrinsics.fy * y) - (v_cy * z);
        }
        ...
        info!("UCM linear estimation results: alpha = {}", self.alpha);

        // Clamp alpha to valid range if needed (note: UCM allows alpha > 1.0)
        if self.alpha <= 0.0 {
            info!("Alpha {} is too small, clamping to 0.01", self.alpha);
            self.alpha = 0.01;
        }
...
impl CameraModel for UcmModel {
    ...
    fn load_from_yaml(path: &str) -> Result<Self, CameraModelError> {
        ...
        // Use helper function to parse YAML (UCM has 5 params: fx, fy, cx, cy, alpha)
        let (intrinsics, resolution, extra_params) = yaml_io::parse_yaml_camera(path, 5)?;

        // Extract alpha from extra_params
        if extra_params.len() != 1 {
            return Err(CameraModelError::InvalidParams(format!(
                "UCM model expects exactly 5 parameters (fx, fy, cx, cy, alpha), got {}",
                4 + extra_params.len()
            )));
        }
    ...
    fn save_to_yaml(&self, path: &str) -> Result<(), CameraModelError> {
        ...
        // Use helper function to save YAML (alpha as extra parameter)
